...
## Reasoning
- Added the `quarkus.platform.group-id` and `quarkus.platform.version` properties to specify the Quarkus platform version.
- Added the `quarkus-maven-plugin` plugin to enable Quarkus-specific build and code generation goals.
- Updated the `maven-compiler-plugin` plugin to use the same source and target versions as before.
- No changes were made to the `maven-war-plugin` plugin as it is not specific to Quarkus.
